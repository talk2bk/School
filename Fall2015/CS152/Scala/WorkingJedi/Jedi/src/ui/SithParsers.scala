...
class SithParsers extends RegexParsers {
 ...
 def sum: Parser[Expression] = 
  ...
  def term: Parser[Expression] = assignment | iteration | deref | lambda | block | literal | identifier | "("~>expression<~")" | failure("Invalid expression")
  ...
  {
    case "["~x~"]" => FunCall(Identifier("val"), List(x))
  }
}